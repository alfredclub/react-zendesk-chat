	...
	switch (action.type) {
		...
		case 'chat':
			...
			switch (action.detail.type) {
				...
				case 'chat.msg':
					...
					new_state.chats = state.chats.concat(_defineProperty({}, '' + action.detail.timestamp + action.detail.msg_id, _extends({}, action.detail, {
		...
		case 'history_loaded':
			var _action$detail = action.detail,
			    loaded = _action$detail.loaded,
			    chats = _action$detail.chats;
