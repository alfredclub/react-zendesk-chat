...
function update() {
	...
	switch (action.type) {
		...
		case 'chat':
			...
			switch (action.detail.type) {
				...
				case 'chat.msg':
					...
					var _action$detail = action.detail,
					    msg_id = _action$detail.msg_id,
					    timestamp = _action$detail.timestamp;

					var id = msg_id ? '' + timestamp + msg_id : '' + (timestamp + 1);

					new_state.chats = state.chats.concat(_defineProperty({}, id, _extends({}, action.detail, {
		...
		case 'history_loaded':
			var _action$detail2 = action.detail,
			    loaded = _action$detail2.loaded,
			    chats = _action$detail2.chats;


			return _extends({}, state, {
				history: { loaded: loaded, chats: chats }
			});
		case 'clean_chats':
			return _extends({}, state, {
				chats: (0, _sortedMap2.default)()
		...
		case 'history_chats':
			return _extends({}, state, {
				historyChats: _extends({}, state.historyChats, _defineProperty({}, action.detail.id, action.detail.messages))
			});
		default:
...
function storeHandler() {
	...
	if (action.type === 'synthetic') {
		...
		var new_timestamp = state.last_timestamp ? state.last_timestamp + 1 : new Date().getTime();
